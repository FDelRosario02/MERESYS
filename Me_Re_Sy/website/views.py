...
from .forms import SignUpFrom, PredictForm
import numpy as np
import pandas as pd
import pickle
...
def helper(dis):
    ...
    die_list = diets[diets['Disease'] == dis][['Diet_1', 'Diet_2', 'Diet_3', 'Diet_4','Diet_5']]
    die_list = [col for col in die_list.values]

    # med_list = medications[medications['Disease'] == dis]['Medication'].values.tolist()
    #die_list = diets[diets['Disease'] == dis]['Diet'].to_list()
...
#         user_symptoms = [s.strip() for s in symptoms.split(',')]
#         user_symptoms = [symptoms.strip("[]' ") for symproms in user_symptoms]
#         predicted_disease = get_predicted_value(user_symptoms)

#         desc, pre, med, die, wrkout = helper(predicted_disease)
#     return render(request, 'home.html',{'predicted_disease' : predicted_disease,'dis_des': desc, 'dis_pre':pre,'dis_med': med, 'dis_wrkout':wrkout})
